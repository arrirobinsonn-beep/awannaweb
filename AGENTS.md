...
### Fix 19 September 2026 — /product: produk & varian berikutnya tidak bisa disimpan tanpa hard refresh
- **Gejala**: setelah menyimpan 1 produk, membuka modal "＋ Tambah Produk" lagi untuk produk berikutnya → tombol `#pm-save` tertinggal **`disabled` bertuliskan "Menyimpan..."** sehingga tidak bisa diklik (pengguna harus Ctrl+Shift+R dulu). Sama untuk `#pv-save` saat menambah varian kedua.
- **Akar masalah**: jalur SUKSES handler `#pm-save`/`#pv-save` hanya `fetchFiltered(); closeProductModal();` — TIDAK pernah mengembalikan `disabled=false`/label (hanya jalur error yang me-reset). Halaman `/product` menyimpang dari pola mayoritas proyek yang benar: `purchase/index.blade.php` & `courier_rule/index.blade.php` memakai `.finally(() => { btn.disabled=false; btn.innerHTML=… })`.
- **Fix** (`resources/views/product/index.blade.php`): helper `resetProductSaveButton()`/`resetVariantSaveButton()` di-reset saat modal DIBUKA (`openProductModal`/`openVariantModal`) DAN di `.finally(…)` setelah simpan. **Label tombol TIDAK diubah** — tetap "💾 Simpan Produk"/"💾 Simpan Varian" (keputusan user: "simpan saja sudah cukup"); yang penting tombolnya berfungsi lagi.
- **Bonus fix**: tabel di-refresh AJAX sedangkan `openRows` (Set global di blok script terpisah) masih menyimpan id → baris varian yang tadi terbuka jadi tertutup & klik PERTAMA tombol 🔖 tidak membuka apa pun. `reopenVariantRows()` (dipanggil di `fetchFiltered()` & `fetchPage()`) membuka kembali baris yang tadi terbuka → tambah varian beruntun jadi mulus.
- **Regresi e2e**: `tests/e2e/product-create-again.spec.ts` (baru) — tambah 2 produk berurutan + 2 varian pada produk yang sama TANPA reload, assert tombol `enabled` + label sudah kembali normal (`Simpan Produk`/`Simpan Varian`), lalu cleanup produk uji lewat UI (accept confirm). Sebelum fix: gagal `element is not enabled`.
- **Pre-existing (BUKAN akibat perubahan ini)**: 6 test di `tests/e2e/product-filter.spec.ts` gagal karena locator basi `#product-table-wrap` / `#product-count` / `#product-pagination` — id itu sudah tidak ada sejak halaman dipisah 2 section (`#core-table-wrap`/`#consumable-table-wrap`, `#core-count`/`#consumable-count`). Belum diperbaiki.
...
### Fix 19 September 2026 — Migrasi `ad_phase` idempotent + unique `regional_cs_stats` tertinggal dari `product_status`
- **Gejala**: `php artisan migrate` mati di `2026_09_18_000000_add_ad_phase_to_regional_tables` → **1060 Duplicate column name 'ad_phase'**. Akar masalah: DDL MySQL TIDAK transaksional — run sebelumnya sudah menambahkan kolom/index unique di `regional_reports` + kolom/index di `regional_cs_stats`, lalu gagal di langkah TERAKHIR (`DROP INDEX cs_stats_unique` → **1091**, karena index itu sudah bernama `cs_stats_status_unique` sisa implementasi `product_status` dari `origin` yang filenya dihapus saat rebase tapi efeknya masih ada di DB); baris `migrations` tidak pernah dibuat → run berikutnya mengulang dari langkah 1.
- **Fix**: migrasi ditulis ulang **IDEMPOTENT** — guard `Schema::hasColumn`, index dicek per nama & per kolom; `alignUnique()` membuang unique LEGACY apa pun yang memuat kolom identitas (`regional_unique` 3 kolom, `cs_stats_status_unique` versi `product_status`) lalu memasang unique fase dalam **1 ALTER atomik**; `mergeDuplicates()` menggabung baris ganda (lead/paid dijumlahkan + `paid_ratio` dihitung ulang) SEBELUM unique dipasang (mencetak peringatan ke output); `down()` juga idempotent + `ensureNamedColumnIndex()` dulu agar FK tidak pernah kehilangan index penopang.
- **Jebakan 1 — `lead` reserved word MySQL**: ilegal juga sebagai **ALIAS** (`... as lead` → 1064), bukan hanya sebagai kolom. Jumlahkan lewat query builder (`->get(['lead','paid'])->sum('lead')`) atau alias non-reserved.
- **Jebakan 2 — `migrate --pretend` TIDAK bisa dipakai memverifikasi migrasi ini**: di dalam `DB::pretend()` hasil `SHOW INDEX` selalu **0 baris** → rencana DDL yang dicetak (drop/ADD index) menyesatkan. Verifikasi HARUS dijalankan sungguhan di DB.
- **Catatan MySQL**: ALTER rebuild yang menambah index fase membuat MySQL membuang index implisit FK yang jadi redundan (nama `*_foreign` bisa hilang di DB tertentu) — FK tetap utuh karena index fase berawalan `user_id`/`cs_user_id`. Nama index beda antar-DB itu normal.
- **Verifikasi**: `laravel_app` (hibrida/partial) & `webawanna_test` (legacy penuh) → unique fase terpasang benar, `up()×2`/`down()×2` idempotent, 18 baris lama utuh, merge terbukti (3 baris running 10/5 + 20/15 + 5/0 → 35/20/57.14%, baris testing tidak ikut tergabung). Suite **255 pass (1331 assertions)**.
- **Sisa (belum dibersihkan)**: `regional_cs_stats.product_status` + index `regional_cs_stats_product_status_index` + baris migrasi hantu `2026_09_10_000000_add_product_status_to_regional_cs_stats_table` (filenya sudah tidak ada di repo) masih ada di DB lokal — tidak mengganggu (kode tidak mengisinya, default `running`).
