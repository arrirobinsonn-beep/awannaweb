...
- **Latar**: user melakukan `git pull --rebase origin parhan`. Commit kerja siang kita = `aab794f` ("perbaikan logika produk testing untuk halaman spending dan detail per daerah", 26 berkas) di-rebase di atas `b1bd6b5` (origin/parhan = `3e80609` + merge `staging`).
- **Yang terjadi**: origin ternyata sudah punya implementasi PARALEL fitur yang sama (`3e80609`, 10 Sep) memakai kolom **`product_status`**, sedangkan kerja kita memakai **`ad_phase`** (dua matriks regional + 2 tabel performa team + product deletion). Git meng-auto-merge berkas yang saling tak bertabrakan → terbentuk kode hibrida rusak (mis. `TeamController::performance()` punya dua closure `$totalOf`/`$sum` + `$totalPerCs` tak terdefinisi; `RegionalController::savePreview` memakai key `ad_phase` TAPI menyimpan `product_status`; `RegionalCsStat` punya `$fillable` `product_status`).
- **Keputusan**: pendekatan `ad_phase` (kerja lokal) yang dipakai; berkas implementasi `product_status` dipulihkan ke versi kita + artefaknya dihapus.
- **Berkas di-`checkout aab794f`** (kembali ke versi kita): `RegionalController`, `TeamController`, `RegionalCsStat`, `RegionalImportService`, `regional/index.blade.php`, `team/performance.blade.php`, `RegionalImportTest`, `MEMORY.md`.
- **Berkas dihapus**: `database/migrations/2026_09_10_000000_add_product_status_to_regional_cs_stats_table.php`, `resources/views/team/partials/performa-donut.blade.php`, `tests/Feature/TeamPerformanceTest.php` (versi kita: `_donut.blade.php` + `TeamPerformanceDualPhaseTest.php`). Kedua migration `product_status` ternyata **Pending** di DB dev & test → aman dihapus tanpa cleanup DB.
...
- **Latar**: user uji input 2–4 Sep; produk NSR (Nozzle Sprayer 2000 Lubang, `start_running=2026-09-04`) — Spending mengategorikan 4 Sep = running (benar), Detail Per Daerah justru mendeteksi LEBIH SEDIKIT data running; selisih = persis lead/paid NSR tanggal 4 Sep.
- **Akar masalah (terbukti via tinker)**: `Product::pluck('start_running')` mengembalikan **objek Carbon** (kolom berkast `date`), lalu `$tanggal >= $runningStart` membandingkan string `'2026-09-04'` vs `'2026-09-04 00:00:00'` — string lebih pendek (prefix) dianggap LEBIH KECIL → baris di hari EXACT produk mulai running dianggap testing → di-skip. H-1 & H+1 benar; hanya hari H yang hilang.
- **Empiris**: `'2026-09-04' >= $carbon->start_running` → `false`; dgn `->toDateString()` → `true`. (DateTime MENTAH lebih parah lagi: tanpa `__toString`, PHP bandingkan sbg int → string vs objek SELALU false.)
- **Mengapa Spending aman**: `Product::phaseOn()` memanggil `->toDateString()` di kedua sisi. **Mengapa lolos test**: test fase memakai 1 Sep vs 3 Sep (start 2 Sep) — tak ada yang uji tanggal exact sama.
- **Fix**: `RegionalImportService` normalisasi peta start_running → string `Y-m-d` (`->map(fn ($v) => $v instanceof CarbonInterface ? $v->toDateString() : (string) $v)`). **Aturan umum**: bandingkan tanggal SELALU string `Y-m-d` di kedua sisi; jangan pernah bandingkan string vs Carbon/DateTime.
- **Test**: +`test_row_on_exact_start_running_date_counts_as_running` (H & H+1 dihitung, skipped_testing=0). Suite **248/248 pass (1300 assertions)**.

## Session: 17 September 2026 — Regional Melewatkan Semua Data "Testing" — Akar Masalah DATA `start_running`, Bukan Kode (fix backfill)

## Session: Regional Melewatkan Semua Data "Testing" — Akar Masalah DATA `start_running`, Bukan Kode (fix backfill)

- **Latar**: user lapor halaman Detail Per Daerah (`/regional`) melewatkan SELURUH data produk running saat upload data mentah — semua terkategori testing.
- **Reproduksi**: `RegionalImportService::parseExcel('training/DataDariOrderOnline(mentah).csv')` → `skipped_testing=5869` dari 5869 baris (semua!). Produk di file tercocok semua (0 gagal match) — matcher BUKAN masalah.
- **Akar masalah = DATA, bukan kode**: klasifikasi fase (fitur W) membandingkan **tanggal order vs `products.start_running`**. File mentah berisi order **7–29 Juli 2026**, sementara backfill seragam fitur W men-set `start_running = 2026-09-01` untuk semua produk → semua order Juli "masa testing" → dilewati. Bug yang sama diam-diam memengaruhi tab Spending & discrepancy (logika fase sama).
- **Keputusan user (Q&A)**: 6 produk inti (KMP, KSP, KBJ, KCHP, SH, KNGH) sebenarnya sudah running sejak **2026-07-01** (sebelum file Juli) — dipilih dari opsi 1 Jul / 1 Jan / per produk.
- **Fix**: UPDATE `start_running='2026-07-01'` hanya utk 6 kode inti (produk testing September & tanggal manual admin tidak disentuh) → re-parse file asli: **skipped_testing=0**, semua baris running. Seeder: `ProductSeeder::RUNNING_SINCE_BY_CODE` (const, 6 kode → 2026-07-01) dipakai sebagai sumber `start_running` bila kosong (DB fresh konsisten dgn produksi).
- **Probe tips**: `ProductNameMatcher::buildIndex()` hanya men-select `id`+`name` — atribut lain (code/start_running) kosong di objek hasil match; ambil ulang via `Product::whereIn('id', ...)` utk inspeksi. `match()` signature: `match(string $rawName, array $index)` dgn index dari `buildIndex()` (bukan Collection).
- **Pelajaran**: backfill tanggal fase yang seragam & di masa depan relatif thd data historis = jebakan retroaktif; klasifikasi by tanggal hanya benar kalau tanggal fasenya benar. Verifikasi klaim "produk running" selalu cek `start_running` aktual (`Product::groupBy start_running`) vs rentang tanggal data sebelum menyimpulkan bug kode.
- **Test**: suite penuh **247/247 pass (1293 assertions)** setelah perubahan seeder.

## Session: Hard Delete Produk + Kode Bisa Dipakai Ulang + Arsip Label Spending (keputusan user)

- **Latar**: user lapor hapus produk lalu tambah produk baru dengan kode sama ditolak "kode produk sudah ada", padahal produk sudah dihapus. Minta juga: saat produk dihapus, data spending seluruh advertiser yang membawakan produk itu berubah label jadi "{nama produk}(Produk dihapus)" agar tidak memengaruhi perhitungan keuangan.
- **Akar masalah 2 lapis**: (1) `Product` memakai SoftDeletes → baris masih ada; (2) `products.code` **UNIQUE di level DB** → walau validasi di-skip utk baris trashed, INSERT tetap ditolak MySQL. Kode lama hanya bisa bebas jika barisnya benar-benar dihapus.
- **Keputusan user (Q&A)**: **Hard delete** (BUKAN arsip/soft delete) + rename spending **semua advertiser** sekaligus (bukan scope milik-sendiri). Saya sampaikan konsekuensinya: HPP laporan operasional untuk order lama jadi 0 (products.purchase_price hilang), jurnal/pembelian kehilangan tautan — disetujui.
- **Jebakan tersembunyi yang dicegah**: `spending_harians.product_id` NOT NULL + **cascadeOnDelete** → hard delete produk lama otomatis MENGHAPUS semua baris spending (bencana keuangan). Migration `2026_09_17_000000_product_hard_delete_prepare_spending` mengubahnya jadi **nullable + nullOnDelete** + kolom baru `product_label` (string nullable) + backfill label = nama produk existing + purge produk lama yang ter-soft-delete (via service, sebelum drop kolom) + **drop `products.deleted_at`**.
- **`ProductDeletionService::delete($product)`** (1 transaksi): (1) UPDATE batch `spending_harians SET product_label = '{nama}(Produk dihapus)' WHERE product_id = ?` SEBELUM delete (anti N+1, jangkauan semua advertiser); (2) hapus manual `packaging_rules` sebagai TARGET (FK cascade hanya utk source; kombinasi multi-gudang); (3) `$product->delete()` → cascade FK: variants → stock_movements/product_variant_inventory/purchases; pivot product_inventory; packaging_rules source. nullOnDelete melepas tautan: shipping_orders.product_id/variant_id, bank_transfers.product_id, spending_harians.product_id (baris selamat, nominal tidak berubah). Return `{product_name, spending_relabelled}` untuk pesan flash.
- **ProductController**: `destroy(Product, ProductDeletionService)` pakai service + pesan "berhasil dihapus permanen (+ N baris spending ditandai)"; `store` set `product_label = name`; `update` menyelaraskan `product_label` bila nama berubah (`wasChanged('name')` — baris yatim tidak tersentuh karena product_id-nya null); validasi code tetap `unique:products` biasa (tanpa trashed, cukup).
- **Tampilan**: `SpendingHarian::display_name` accessor (product ? name : product_label ?? 'Produk Dihapus') + `is_orphan_label`; form edit spending kirim `product_id: null` utk baris yatim (`?? 'null'` di JSON embed — JS `String(null)` aman, form tetap bisa di-save dgn pilih produk lain); view advertiser (Running & Testing, 2 salinan identik per blok) + `_table_general` fallback `$prodData['product']->name ?? ($prodData['whitelists']->first()->display_name ?? 'Tidak Diketahui')` + kode disembunyikan bila produk null.
- **Trap teknis**: (1) table `spending_harians` TIDAK soft-deletes di semua row yang di-update — UPDATE batch di service mengabaikan trashed? TIDAK: Eloquent `update()` pada query model otomatis exclude trashed → justru benar (baris spending yang sudah dihapus user tidak di-resurrect labelnya); (2) Laravel 12 `change()` native + FK harus di `Schema::table` terpisah; (3) setelah drop SoftDeletes dari model, jangan pakai `withTrashed()` di migration purge — query biasa (kolom deleted_at belum di-drop saat purge jalan); (4) test memakai `RefreshDatabase` adalah KESALAHAN di project ini (DB test `webawanna_test` tanpa refresh, data menumpuk) — pakai entitas uniqid + cleanup finally; (5) `assertStringNotContainsString('Tidak Diketahui')` terlalu ketat — string itu ada sebagai literal fallback JS di halaman + baris yatim run sebelumnya.
- **Test**: `ProductDeletionTest` baru 6 test (hard delete + kode reuse via HTTP store, relabel spending lintas advertiser + nilai spending tidak berubah, service count, cascade varian/jurnal + order selamat dgn amount utuh, packaging rule source cascade & target manual, halaman spending menampilkan label arsip). Suite penuh **247/247 pass (1293 assertions)**. Migration dijalankan di DB aplikasi `webawanna` & DB test `webawanna_test`.

# MEMORY — 9 September 2026

## Session: Fase Iklan Produk Berbasis Tanggal — start_testing & start_running (fitur W)

- **Latar**: user minta klasifikasi spending iklan Testing/Running mengikuti TIMELINE produk, bukan `ad_status` saat ini — masalah: produk di-toggle running belakangan membuat spending masa testing ikut pindah ke Running. Jawaban Q&A user: (1) lifecycle SATU ARAH (testing→running, tidak bisa balik) → cukup 2 kolom tanggal; (2) backfill seragam existing → start_testing=2026-08-01 & start_running=2026-09-01; (3) tanggal fase bisa diedit manual di form produk.
- **Analisa**: pendekatan timestamp + klasifikasi by tanggal saat query LEBIH BAIK daripada snapshot status per baris spending (retroaktif benar walau upload telat, tanpa duplikasi data, single source of truth). Kunci: kalau toggle bolak-balik → butuh tabel riwayat fase; karena satu arah → 2 kolom cukup.
- **Implementasi inti**: migration `2026_09_09_000000` (start_testing/start_running date + index + backfill); `Product::phaseOn($date)` (start_running null→testing; tanggal>=start_running→running; banding string Y-m-d); `toggleAdStatus` satu arah (running→testing 422, testing→running set start_running=today); ProductController create set start_testing=today, form edit terima 2 tanggal (nullable date, pertahankan nilai lama bila tidak dikirim).
- **Klasifikasi date-aware di SEMUA titik**: indexAdvertiser & indexGeneral (`phaseOn($r->tanggal)`), chart 4 garis & tab splits di view (`$phaseOn = fn($prod,$d)` + closure `use ($phaseOn)` — closure di Blade TIDAK menangkap variabel luar tanpa use), badge Testing di `_table_general` (`phaseOn($dateKey)`), query discrepancy running-only di 4 titik jadi JOIN (`join products` + `whereNotNull(start_running)` + `whereColumn(spending_harians.tanggal >= products.start_running)` + kolom berkualifikasi), RegionalImportService (`pluck('start_running','id')` + klasifikasi per tanggal baris), ProductSeeder isi start_* bila kosong.
- **View produk**: running → badge saja (tanpa toggle, karena satu arah) + "sejak {d M Y}"; testing → toggle + badge + "sejak"; modal edit + 2 input date + data attrs `data-start-testing/running`.
- **Test**: helper makeProduct 5 file set `start_testing='2026-01-01'` + `start_running` sesuai status (tanpa itu phaseOn=testing → summary running gagal); +3 test baru: fase split by tanggal (`data-run="Rp 50.000"` vs `data-test="Rp 100.000"`), toggle satu arah, regional fase (baris testing phase dilewati walau produk sekarang running).
- **Fix flakiness stok test (bonus penting)**: 4 test packaging gagal karena `KMP+1.00` (varian default) stok 0 — terkuras oleh run-run sebelumnya di DB test tanpa refresh; `seedVariant` firstOrCreate tidak restore stok yang terkuras. Fix: `ensureCatalog()` di OrderOnlineTest kini balikkan SEMUA jurnal `order_online` (`reverseReferences` + recalc) → tiap test mulai dari baseline katalog. Pola ini berlaku untuk flakiness stok serupa di masa depan.
- **Hasil**: suite **241/241 pass (1261 assertions)**. AGENTS.md section W ditulis.

## Session: Analisa menyeluruh proyek + fix 16 test gagal (route hilang, test basi, vendor, idempotensi)

- **Latar**: user minta analisa seluruh file/kode/folder setelah lama tidak dikerjakan. Hasil analisa: suite 16 failed / 222 passed → setelah fix **238/238 pass (1242 assertions)**.
- **Akar masalah #1 — 4 route `.filter` HILANG** (500 di halaman live, 12 test gagal): `courier-rule.filter`, `product.filter`, `supplier.filter` hilang saat merge `staging` ke `parhan` (masih ada di `origin/staging`); `tracking-status-rule.filter` TIDAK PERNAH ada di branch mana pun (view + controller method ada, route tidak). View yang manggil: `courier_rule/index` (L235), `product/index` (L231), `supplier/index` (L48), `tracking_status_rule/edit` (L351). Fix: 4 route ditambahkan ke `routes/web.php` (posisi `tracking-status-rule.filter` = GET `/tracking-status-rules/{source}/filter` di dekat route lain).
- **Akar masalah #2 — test basi AJAX JSON**: `CourierRuleController` & `TrackingStatusRuleController` sudah pindah ke response JSON (fetch no-reload, commit "perbaikan tampilan bir ga reload realod") tapi test lama masih `assertRedirect`/`assertSessionHasErrors`. Fix: 11 titik assertion di `CourierRuleTest` + 2 di `TrackingStatusRuleTest` → `assertOk()->assertJson(['success' => true])` / `assertStatus(422)->assertJson(['success' => false])`. Catatan: `test_store_requires_valid_courier` tetap pakai `assertSessionHasErrors` (validasi tanpa header JSON → redirect, masih benar).
- **Akar masalah #3 — dashboard redesign**: `dashboard/general.blade.php` di-redesign (commit "perbaikan tampilan admin versi parhan") → kartu berubah dari "Barang Keluar/Masuk Hari Ini, Resi Hari Ini, Metode Pembayaran" ke "Order Hari Ini, Stok Hari Ini, COD vs Bank Transfer" (+Revenue, Pengajuan Pending). Fix assertion di `OperationalReportTest::test_dashboard_general_shows_ops_today_cards`.
- **Akar masalah #4 — vendor tidak sinkron**: `barryvdh/laravel-dompdf` ada di composer.lock tapi belum ter-install → `Rekening-Koran PDF` 500 (`Class "Barryvdh\DomPDF\Facade\Pdf" not found`). Fix: `composer install`.
- **Akar masalah #5 — test move courier tidak idempotent**: `test_move_down_swaps_sort_order` pakai `sort_order` FIX 50/51 + cleanup di akhir (bukan finally). Run gagal → sisa rule `TEST PROVINCE` sort_order 50/51 menumpuk di DB test → run berikutnya `move()` swap dengan SISA rule, bukan `$b`. Fix: (1) hapus 23 sisa rule `TEST PROVINCE %` dari `webawanna_test`; (2) test pakai `$base = max+10` dinamis (jamin adjacency) + `try/finally` cleanup.
- **Catatan penting DB test**: `phpunit.xml` punya `<env DB_DATABASE=awannatestlaravel force=true>` (DB lama yang SUDAH TIDAK ADA di MySQL) TAPI `<server DB_DATABASE=webawanna_test>` yang MENANG — Laravel baca `$_SERVER` (ServerConstAdapter) lebih dulu dari `$_ENV`. Jadi test tetap jalan di `webawanna_test` (sesuai AGENTS.md). Bila mau bersihkan, hapus entri `awannatestlaravel` dari phpunit.xml.
- **Hasil**: suite **238/238 pass (1242 assertions)** — ExampleTest 302 pre-existing pun ikut hijau.
