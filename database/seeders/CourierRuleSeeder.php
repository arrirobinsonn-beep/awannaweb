...
class CourierRuleSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // 3. COD → flix-sicepat (seluruh Jawa + Bali)
        foreach ([
            'BANTEN', 'DKI JAKARTA', 'JAWA BARAT', 'JAWA TENGAH', 'JAWA TIMUR', 'DI YOGYAKARTA', 'BALI',
        ] as $province) {
            $rules[] = ['sort_order' => $sort++, 'payment_method' => 'cod', 'province' => $province, 'courier' => 'flix-sicepat'];
        }

        // 4. COD → flix-spx (pulau lainnya di luar flix-idx & flix-sicepat)
        foreach ([
